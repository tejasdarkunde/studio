...
type EditParticipantDialogProps = {
  isOpen: boolean;
  onClose: () => void;
  onSave: (details: Participant) => void;
...
export function EditParticipantDialog({ isOpen, onClose, onSave, participant }: EditParticipantDialogProps) {
  ...
  useEffect(() => {
    if (isOpen && participant) {
      ...
      setIitpNo(participant.iitpNo || '');
      setMobile(participant.mobile || '');
      setOrganization(participant.organization || '');
      setEnrolledCourses(participant.enrolledCourses?.join(', ') || '');
    }
  }, [isOpen, participant]);
    ...
    <Dialog open={isOpen} onOpenChange={(open) => { if(!open) onClose()}}>
      <DialogContent className="sm:max-w-[480px]">
        <DialogHeader>
          ...
          <DialogDescription>Update the details for {participant?.name}.</DialogDescription>
        </DialogHeader>
